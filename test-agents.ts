...
test('ClaudeCodeChecker.check() returns installed:true with version when claude --version succeeds', async t => {
	const checker = new ClaudeCodeChecker((cmd: string) => {
...
test('ClaudeCodeChecker.check() falls back to which and returns path when --version fails', async t => {
	const checker = new ClaudeCodeChecker((cmd: string) => {
...
test('ClaudeCodeChecker.check() returns installed:false when both commands fail', async t => {
	const checker = new ClaudeCodeChecker(() => {
		throw new Error('not found');
	});
	const result = await checker.check();
...
test('OpenCodeChecker.check() returns installed:true with version when opencode --version succeeds', async t => {
	const checker = new OpenCodeChecker((cmd: string) => {
...
test('OpenCodeChecker.check() falls back to which and returns path when --version fails', async t => {
	const checker = new OpenCodeChecker((cmd: string) => {
...
test('OpenCodeChecker.check() returns installed:false when both commands fail', async t => {
	const checker = new OpenCodeChecker(() => {
		throw new Error('not found');
	});
	const result = await checker.check();
