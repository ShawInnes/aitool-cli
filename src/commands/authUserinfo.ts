...
export async function runAuthUserinfo(
	configDir?: string,
	...
	if (!discovery.userinfo_endpoint) {
		throw new Error(
			'The configured identity provider does not expose a userinfo endpoint.',
		);
	}
	...
	if (response.status === 401) {
		throw new Error(
			'Access token is invalid or expired. Run `aitool auth login` to re-authenticate.',
		);
	}

	if (!response.ok) {
		throw new Error(
			`Userinfo request failed (${response.status}): ${discovery.userinfo_endpoint}`,
		);
	}
